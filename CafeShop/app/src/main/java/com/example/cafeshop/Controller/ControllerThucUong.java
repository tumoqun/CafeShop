...
public class ControllerThucUong {
    ...
    public  void getDanhSachThucUongController(NestedScrollView nestedScrollViewThucUong, RecyclerView recyclerViewThucUong, final ProgressBar progressBar){
       ...
       final List<ThucUongModel> thucUongModelList = new ArrayList<>();
        ...
        adapterRecyclerThucUong.setOnItemClickListener(new AdapterRecyclerThucUong.OnItemClicklistener() {
            @Override
            public void OnPlusClick(int position) {
                GioHangModel.Instance().ThemVaoGio(thucUongModelList.get(position));
                Toast.makeText(context,GioHangModel.Instance().SoLuong()+" "+GioHangModel.Instance().ThongTinDoUong()+" "+GioHangModel.Instance().TinhTongGia(),Toast.LENGTH_SHORT).show();
            }

            @Override
            public void OnMinusClick(int position) {
                GioHangModel.Instance().XoaKhoiGio(thucUongModelList.get(position));
                Toast.makeText(context,GioHangModel.Instance().SoLuong()+" "+GioHangModel.Instance().ThongTinDoUong()+" "+GioHangModel.Instance().TinhTongGia(),Toast.LENGTH_SHORT).show();
            }
        });
